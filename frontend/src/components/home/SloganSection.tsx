...
function SloganSection() {
    ...
    return (
        <Box className="h-screen" component={motion.div} width={"100%"} display={"flex"} flexDirection={"column"} justifyContent={"center"} alignItems={"center"} gap={2} whileInView={{ y: 0, opacity: 1 }} initial={{ y: 15, opacity: 0 }} transition={{ duration: 0.5 }} viewport={{ once: true }}>
            ...
            <Button href="/shop" className={`normal-case! ${theme === "light" ? 'text-zinc-100! bg-linear-to-r from-blue-500 to-blue-300' : 'text-zinc-700! bg-zinc-100!'} pl-4! pr-4! pt-2! pb-2! hover:bg-zinc-200!`} color="inherit" draggable={false} endIcon={<ArrowRightAltOutlined/>}>Shop</Button>
        </Box>
    );
}
